...
> `M1_CORPUS_PATH` (falling back to the sibling m1-example example project).
...
`../m1-example/UQR-EV/01.00/Project.m1prj` (or `$M1_PROJECT`); corpus via
...
```bash
cargo build --release
M1_PROJECT=../m1-example/UQR-EV/01.00/Project.m1prj ./target/release/m1-typecheck --audit-names 2>/dev/null | head
```
